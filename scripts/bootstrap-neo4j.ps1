...
function Download-Archive {
  ...
  # BLOCK 7: Reuse a pinned archive if it already exists locally.
  # WHY: First run may need the network, but later launches should not redownload large third-party files every time.
  if (Test-Path -LiteralPath $ArchivePath) {
    return
  }

  # BLOCK 8: Download through a temporary file, then move it into place.
...
function Expand-ArchiveIfNeeded {
  ...
  # BLOCK 9: Skip extraction when the expected tool folder is already present.
  ...
  # BLOCK 10: Expand the pinned archive into user/tools.
...
function Get-JavaHome {
  # BLOCK 11: Find the extracted portable Java runtime by its java.exe.
  ...
  # BLOCK 12: Stop early if Java extraction did not produce the expected executable.
...
function New-Neo4jPassword {
  # BLOCK 13: Generate a local random password without using special characters.
...
function Read-Neo4jConnection {
  # BLOCK 14: Read the existing connection file if one already exists.
...
function New-Neo4jConnection {
  # BLOCK 15: Refuse to invent a new password for an existing database.
  ...
  # BLOCK 16: Create the local connection contract with a generated password.
  ...
  $connection | ConvertTo-Json | Set-Content -LiteralPath $NEO4J_CONNECTION_PATH -Encoding UTF8
  return [pscustomobject]$connection
}

function Write-Neo4jConfig {
  # BLOCK 17: Generate the local Neo4j config that points all mutable state at user/neo4j.
  ...
  # BLOCK 18: Copy Neo4j's non-secret default config support files into the runtime config folder.
  ...
  # BLOCK 19: Build the generated neo4j.conf with only local listeners and ignored runtime directories.
...
function Invoke-WithNeo4jEnvironment {
  ...
  # BLOCK 20: Temporarily point Neo4j commands at the portable Java runtime and generated config.
...
function Test-TcpPortOpen {
  ...
  # BLOCK 21: Check whether the local Bolt port is already accepting connections.
...
function Set-InitialPasswordIfNeeded {
  ...
  # BLOCK 22: Only set Neo4j's initial password during first local bootstrap.
  ...
  # BLOCK 23: Pass the generated password to Neo4j without echoing it.
...
function Start-Neo4jIfNeeded {
  ...
  # BLOCK 24: Reuse a running local Neo4j listener if one is already present.
  ...
  # BLOCK 25: Start Neo4j as a local process, not a Windows service.
  ...
  # BLOCK 26: Wait briefly for the local Bolt listener to accept connections.
...
# BLOCK 27: Prepare ignored folders for downloads, extracted tools, config, and database state.
...
# BLOCK 28: Download and unpack the pinned portable Java runtime.
...
# BLOCK 29: Download and unpack the pinned Neo4j Community archive.
...
# BLOCK 30: Create or reuse local Neo4j credentials and generated config.
...
# BLOCK 31: Set the first password once and start the local database.
