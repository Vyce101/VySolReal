...
Qdrant Vector Store is the shared local vector database that keeps embedded chunk points and manifested graph node vectors with their retrieval metadata. It is the vector store, not the graph store.
...
It also exists so vector persistence can be validated independently from chunk persistence. A chunk can already exist in world storage while its embedding is still pending, stale, or missing. A graph node can already exist in a manifestation manifest while its vector or graph write is still pending. Qdrant gives VySol a dedicated place to confirm that vector-side truth.
...
The vector payload keeps retrieval metadata such as the world UUID, the active ingestion run id, source filename, chunk position, embedding profile details, and the chunk text hash. The full chunk text is not duplicated here. [World Storage](../world-ingestion-pipeline/world-storage.md) remains the source of truth for full text, while Qdrant keeps the vector and the metadata needed to find the right chunk again.
...
When the embedding pipeline starts, it reads the live Qdrant points for the book's expected point ids and compares them against the embedding manifest and the current chunk hashes. If a stored point hash does not match the current chunk text, that point is deleted before overwrite. If the manifest says a chunk is embedded but the point is missing, the chunk is marked for re-embedding. If the saved point belongs to an older ingestion run, that stale point is deleted before the current run overwrites the same chunk slot.
...
## How Graph Node Vectors Also Use Qdrant

Chunk vectors are not the only vectors stored here anymore. During [Graph Manifestation](../world-ingestion-pipeline/graph-manifestation.md), VySol also stores extracted node vectors in separate graph-node collections. Those node vectors keep their own provenance, point ids, and `ingestion_run_id` metadata while still using the same local Qdrant database.

That split matters because chunk vectors and node vectors serve different jobs. Chunk vectors support chunk retrieval. Node vectors support graph-side similarity work later. They share one local vector database, but they do not pretend to be one interchangeable record type.
...
Finally, the choice not to duplicate full chunk text in Qdrant keeps responsibilities clearer. Qdrant is the vector index and retrieval metadata layer. [World Storage](../world-ingestion-pipeline/world-storage.md) is the authoritative content store. [Neo4j Graph Store](neo4j-graph-store.md) is the separate graph layer for nodes and relationships. That split makes recovery and inspection easier because each system has one clear job.
